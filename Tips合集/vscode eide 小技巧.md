...
建议添加 -Wextra 选项打开额外的 warning，方便在写代码时找到 bug. 

eide 可以给不同的源文件附加不同的编译选项，因此如果只想给 UserCode 下的代码添加 -Wextra 选项，那么可以如下操作：

1. 右键项目资源->为源文件附加单独的编译参数

    ![1666971938005](image/vscodeeide小技巧/1666971938005.png)

2. 在 files: 下添加`'./UserCode/**': -Wextra`（注意缩进，否则不会生效）：
    ```yml
    files:
    #   './test/**/*.c': --c99
        './UserCode/**': -Wextra
